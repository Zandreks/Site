...
$nzkomp =  $db-> real_escape_string(htmlspecialchars($_POST["ncomp"]));
$adres =  $db-> real_escape_string(htmlspecialchars($_POST["adres"]));
$pol =  $db-> real_escape_string($_POST["pol"]);
$datayear =  $db-> real_escape_string($_POST["year"]);
...
$error = false;
$coords =  $db-> real_escape_string($_POST['coords']);
...
$from = "info@elit-astana.kz"; // адрес, от которого придёт уведомление, желательно указать существующий ящик на хостинге!
...
if (!$error) {
    ...
    $arr = $_POST["index"];
    $array = implode(", ", $arr);
    $db->query("INSERT INTO `users` (`name`, `sumname`, `email`, `phone`, `pol`,`nzkomp`,`adres`, `datayear`, `datamount`, `dataday`, `password`, `kiy`,`indexs`,`maps`,`active`) VALUES ('$name','$sumname','$email','$phone','$pol','$nzkomp','$adres','$datayear','$datamount','$dataday','$password','$key','$array','$coords','$activation')");
